...
def main(simple: bool):
    with open(f"data/coarse_discourse_dataset{'.simple' if simple else ''}.csv", "w", newline='') as csvfile:
        out = csv.DictWriter(csvfile, dialect="unix", fieldnames=["id", "annot1", "annot2", "annot3"], extrasaction="ignore")
        out.writeheader()
            ...
            for line in fin.readlines():
                ...
                for post in record["posts"]:
                    if len(post["annotations"]) == 3:
                        ...
                        if simple:
                            if item["annot1"] in simple_labels and item["annot2"] in simple_labels and item["annot3"] in simple_labels:
                                out.writerow(item)
                        else:
                            out.writerow(item)
