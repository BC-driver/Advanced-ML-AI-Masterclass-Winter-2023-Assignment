...
def load_dataset(source_data, batch_size):
    ...
    # print(label.shape)
    # label = np.zeros((len(source_data), 4))
    # label = pd.DataFrame(label)
    # for i in range(len(source_data)):
    #     label.loc[i, source_data.loc[i, "color"]] = 1
...
def main():
    source_data = pd.read_csv("set3.csv")
    batch_cnt = 4
    #epoch_cnt = 100
    learning_rate = 1e-3
    ...
    while lastCorrect <= 0.90 and epoch_cnt <= 1500:
        epoch_cnt += 1
        sum_loss = 0.0
        correct = 0.0
    ...
    for i in range(len(source_data)):
        plt.scatter(source_data.loc[i, "x"], source_data.loc[i, "y"], color=PTcolors[source_data.loc[i, "color"]], s=100)
    mat[299, 299] = 3
